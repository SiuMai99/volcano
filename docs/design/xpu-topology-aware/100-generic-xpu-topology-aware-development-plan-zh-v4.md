...
建议按 **合同冻结 → 数据与启用基础 → Advisory MVP → Pod-derived Topology Alpha → 后续 Exact → 发布验收** 推进。
...
完成拓扑 Alpha。跨系统事务、外部 reservation 对账和 crash-safe release 不属于 Alpha，另行排期。
...
| 后续 Exact M4 | 在 Alpha 之上增加 backend durable reservation、完整 batch、跨系统恢复/释放和真实 exact-ID enforcement | 单独完成 coordinator、Adapter conformance、故障注入与真实硬件验收；不反向扩大 Alpha |
| 发布候选 M5 | 安装、升级、关闭、容量与性能边界可复现；分别声明 Alpha 与后续 Exact 能力 | 对应能力矩阵、E2E、性能报告及运维手册通过 |
...
- Alpha 复用现有 Statement 与逐 Pod PreBind/Bind；不新增独立 allocation ledger、EvidenceStore、ActivityFence 或第二套 commit loop。
...
- 中间 PR 保持 feature 默认关闭。外部 reservation、完整 batch、跨系统恢复/释放和 active-active 语义转入后续 Exact。
...
自动推导 Fabric、通信 ring 优化、durable reservation/recovery/release、完整 batch、active-active fencing、workload 同时启动屏障。
...
| [Statement](../../../pkg/scheduler/framework/statement.go) | `Commit()` 无 error；Merge 转移 operations；SaveOperations 克隆 Task operations | Alpha 复用现有 Commit；不复制外部 token/participant；后续 Exact 再评估 error-returning path |
| [Bind cache](../../../pkg/scheduler/cache/cache.go)、[PreBinder](../../../pkg/scheduler/cache/interface.go) | `AddBindTask` 单项入队；`executePreBinds` 保留成功项；rollback 无错误结果 | Alpha 保持逐 Pod Bind，并验证 assignment annotation 随 Bind 保留；完整 batch 属于后续 Exact |
...
| D3：framework 扩展形状 | Alpha 复用现有 Statement 与逐 Pod Bind；完整 exact coordinator、不可拆分 batch 和跨系统事务留作后续，不在 Alpha 冻结 participant 接口 | 后续 Exact |
| D4：首个 Provider | 用 XPU-01 的小实验验证：scheduler-selected DeviceKey 能被 Provider/Device Plugin 消费或确认，并能写入/保留 Pod assignment annotation；durable reservation/release 留作后续 Exact | XPU-01、15 |
...
| D8：组合与状态归属 | 多 SubGroup、多 resource policy 仍在同一 Statement/Session 中求约束交集；每个 Pod 一份 assignment，Group anchor 从已绑定成员重建，不建立共同 reservation | XPU-08、11、13 |
...
topology 和 annotation 模拟；第二层在后续 Exact 前用真实 NVIDIA 节点补 runtime identity 验收：
...
5. 明确 nvml-mock 能覆盖的 Fabric/topology/health 故障与真实 NVIDIA Fabric 验收边界；reservation、release、epoch 另列为后续 Exact。

产物是 NVIDIA Provider 能力表、mock/真实分层日志与断言、尚缺的协议和硬件清单。nvml-mock 能证明身份和拓扑映射行为，不能替代真实
容器 UUID 验收。Provider 不能消费 scheduler-selected DeviceKey 时，Alpha 对相关 hard workload 保持 Pending；durable/release 语义
另列后续 Exact 工程，不阻塞 Pod-derived Alpha 的设计收敛。
...
| XPU-09 | Statement/Session Group plan 与提交分支审计 | 08；D3 | S | 3～5 |
| XPU-10 | 后续 Exact ledger、Adapter contract 与 Fake backend | 06、08；后续 D3 | S/R | 6～9 |
| XPU-11 | Pod-derived anchor、assignment annotation 与恢复 | 03、08；D2/D6 | S/R | 4～6 |
| XPU-12 | 后续 Exact complete-batch PreBind、rollback 与队列接收 | 02；后续 D3 | S | 6～9 |
| XPU-13 | Alpha 提交 guard 与所有 action 入口防绕过 | 08、11；D7/D8 | S | 4～6 |
| XPU-14 | Alpha Pod/Node recovery、eviction 旁路与 drain | 11、13 | S | 4～7 |
| XPU-15 | 后续真实 Exact Adapter 与 runtime ID 验证 | 01、10、12、13/14 联调 | R | 8～14 |
| XPU-16 | Mock/真实 Group、Fabric、失败注入 E2E | 14；后续 Exact 部分依赖 15 | Q/S/R | 6～10 |
| XPU-17 | metrics、性能与并发容量基线 | 07 可启动；Alpha 部分依赖 13/14 | Q/S | 4～6 |
| XPU-18 | 安装示例、升级/drain/回滚与发布清单 | 04、16、17 | A/Q | 3～5 |

Alpha 主线约 **74～110 工程人日**（XPU-00～09、11、13、14、16～18）；后续 Exact 另计 XPU-10、12、15 及其联调。
...
后续 Exact 的 external owner/tombstone 生命周期在 XPU-10/14 单独联测；Alpha 只验证 Pod-derived anchor 的失效与 Pending。
...
`volcano.sh/xpu-assignment` annotation 恢复，复用现有 Statement 与逐 Pod Bind。它不提供跨系统 reservation、
完整 batch 或 crash-safe release；这些能力留在本节末尾的后续 Exact 工作包。
...
- 复用 `SaveOperations/RecoverOperations` 复制可试算状态；不复制 reservation token、participant、handoff owner 或外部事务状态。
- 对已 Ready Group 的新增 Task，先从已绑定 Pod 恢复 anchor，再与本次 Session 的 plan 求交集；不能为新 wave 选择第二个 Domain/Fabric。
- 普通 workload 继续使用现有 `Commit()`；Alpha 不新增 error-returning exact commit 或平行 Statement。
...
### XPU-10：后续 Exact ledger、Adapter 合同与 Fake backend

> 不属于 Pod-derived Alpha。只有在需要 Pod 消失后仍恢复外部 reservation、精确 release 或跨 scheduler/Adapter 崩溃对账时才实施。

**落点**：未来 topology live cache/ledger 与 `pkg/scheduler/topology/adapter/`（建议）。

- 另行冻结 Reserve/Prepare/Commit/Compensate/Release/Recover/Reconcile、稳定 token/digest、epoch 和 handoff identity。
- 规定 Unknown、空结果、缺项、PodNotFound 与 Node replacement 的后续 Exact 语义；不能把“列表缺项”直接当作 Free。
- 通过 Fake 注入跨系统边界失败，证明不会重复分配、错误释放或留下不可枚举的外部 reservation。

**完成条件**：独立 Exact 设计和 conformance suite 通过；不把这些完成条件倒灌为 Alpha 的放行条件。
...
### XPU-12：后续 Exact complete-batch PreBind、rollback 与队列接收

> 不属于 Alpha。Alpha 保持现有逐 Pod PreBind/Bind，因此不声称多 Pod Bind 原子性或全组 rollback。

**落点**：未来 `cache/{cache,interface}.go`、binder registry/BindContext、相关 PreBinder 与测试。

- 另行定义完整 batch 的校验、PreBind、rollback、acceptance、队列重试和 owner handoff。
- 明确该能力不能通过循环调用现有 `AddBindTask()`/`executePreBinds()` 伪造。

**完成条件**：独立 Exact 设计证明任意成员 PreBind 失败时 Kubernetes Bind 调用为 0；不作为 Alpha 条件。
...
- preempt/reclaim/eviction 只改变 Pod 生命周期，不提前建立 reservation，也不把 eviction 当成精确 Released。
...
- 同一 active leader 负责调度路径；不增加 follower reservation、backend lease 或 fencing。
- Pod 删除、eviction、FutureIdle 和 pipeline 不被解释为外部设备已 Released；可用性由现有 cache/provider 更新反映。
- reload/drain 检查 feature、policy mutation 和 Pod 生命周期；不等待不存在的 ledger/evidence manager。

**M3 完成条件**：Session 重启、Pod/Node event、Node replacement、annotation 缺失/冲突和旁路场景均有测试；普通 workload 不受影响；
关闭 Alpha 前不再有受其管理的 active topology Pod，且不需要清理独立 reservation ledger。

### 后续 Exact 集成（XPU-10、XPU-12、XPU-15）

后续 Exact 才实现跨系统 coordinator、durable reservation/evidence、完整 batch、重启对账、精确 release、active-active fencing 和真实
exact-ID enforcement。它们可以复用 Alpha 的 canonical DeviceKey、Pod assignment 和 Group planner，但必须另开合同、故障矩阵和放行条件。

## 7. M4/M5：后续 Exact 验证与发布分级

### XPU-15：后续真实 Exact Adapter
...
- 对真实 backend 实现后续 XPU-10 合同，复用 XPU-01 探针，不在 Filter/Score/Prepare 中重新运行 chooser 替换 DeviceIDs。
- 建立 runtime identity 观测：plan、后续 durable reservation、Pod/container 实际设备逐项关联，输出可审计证据。
- 完成 lease/epoch、幂等操作、进程重启恢复、显式 release 和超时 Unknown；这些不属于 Alpha。
...
**M4 完成条件**：后续 Exact 的 conformance suite 同时跑 Fake 与真实 backend；真实容器 ID 等于 plan、重启恢复不漂移、
释放后才复用、失败注入不泄漏/双分配。Alpha 发布说明只声明 Pod-derived anchor、assignment identity 和已验证的 Node-local/Fabric
映射能力；真实 backend 的 reservation/release 另行标注。
...
- 真实设备：选中 ID、Pod assignment annotation 和 runtime identity；backend crash/restart、release、leader handoff 属于后续 Exact；
  真实 Fabric 作为独立环境项记录。
- Alpha 故障测试断言绑定 Pod 的 NodeName/annotation、anchor 数量、状态和 reason；后续 Exact 再断言 owner 数量与最终可复用时间。
...
- 回滚到不含 xPU core guard 的旧二进制前，必须完成 drain 并处理存量 policy/schema；后续 Exact 的 durable evidence 另有回滚要求。
- V3 `tier/tierName` 用显式 catalog 映射迁移；没有发布过 V3 时也保留输入拒绝测试。
- 发布说明分别列出 Advisory、Pod-derived Topology Alpha、后续真实 Node-local Exact、后续真实 Fabric 的验证范围与不支持请求形状。

**M5 完成条件**：按文档在干净环境可重复安装和演练；存在受保护的已绑定 topology Pod 时语义变更/关闭被拒绝；drain 完成后关闭成功；
发布能力表与测试报告一致。
...
| T16 试算回滚 | trial 无外部调用；Save/Recover/Merge 不复制外部状态；winning placement 唯一 | 09 | 单元 + 故障注入 |
...
| T19 后续 Exact 提交 | Commit/accept 失败或未知、batch dispatch 后部分 Bind 的对账 | 10、12、15 | 后续 Exact 故障注入 |
| T20 后续 Exact 释放 | Allocated/Unknown/空/缺项/PodNotFound 的 durable release 语义 | 10、14、15 | 后续 conformance + E2E |
| T21 抢占/旁路 | eviction/pipeline/backfill/nomination 不绕过 hard；eviction 不直接伪造 Released 或新 anchor | 13、14 | action 回归 + E2E |
| T22 Pod-derived 恢复 | Session restart、两阶段 wave、Node replacement、assignment 缺失/冲突；跨 wave anchor 不漂移 | 11、14、16 | API server + E2E |
| T23 后续真实 ID | runtime ID=plan；普通同 resource 占用可见；重启恢复、Released 和幂等成立 | 15 | 后续真实硬件 |
| T24 观测与回归 | controller blocker 不被 gang 覆盖；resolved 清除；无 policy 决定不变；指标低基数 | 04、07、17 | 集成 + benchmark |
| T25 安装与关闭 | 双进程 gate；受保护 topology Pod/policy activity 时拒绝语义变更或关闭；drain 后关闭；旧版本回滚步骤可执行 | 02、14、18 | 安装/升级演练 |
...
2. 对应测试族通过，检查正常行为与失败后的 Pod annotation、anchor、状态和 reason；后续 Exact 才检查 external owner/资源状态。
...
- 后续 Exact：另计 XPU-10、12、15 及其故障/硬件验收，不能与 Alpha 日期合并承诺。
...
| framework | 09 的 Statement/Session 审计与 11 的 Pod-derived anchor 可在 planner 后并行 | 09/11/13/14 完成才能连接 Alpha；12 的 batch contract 属于后续 Exact |
| Adapter | 01 提前做 Provider identity probe；15 的真实 Exact 与 Alpha 并行但不阻塞 Alpha | 真实 Exact 验收依赖 10/12/15；不能用 Fake 代替 |
...
| 第 4～5 天 | 评审最小 Alpha 提交路径、旁路 guard 与 quiescent 规则；将完整 batch/ledger 标为后续 Exact | 决策修订、恢复/冲突序列表、首批 PR 的明确边界 |
...
| 风险 | 尽早获得的证据 | 调整方式 |
| --- | --- | --- |
| backend 不能指定/确认 DeviceKey | XPU-01 的 Provider 探针 | Alpha 对相关 hard workload 保持 Pending；将 exact-ID enforcement 单列为后续工程 |
| framework review 不接受新增 hook/participant 形状 | XPU-00 的最小 Alpha 合同 | 保持现有 Statement/逐 Pod Bind；不为 Alpha 引入平行事务层 |
...
| 关闭配置/旧版本回滚存在受保护 topology Pod | T25 drain 演练 | 完成 Pod/policy drain 后关闭；Alpha 不要求清空独立 ledger，后续 Exact 另行处理外部 owner |
