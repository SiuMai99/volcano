...
The cache serializes work per provider and Node. It records the current Node name and UID seen from the informer, and accepts an update only when that Node incarnation still matches. A same-UID metadata update, including a label or taint change and its new `resourceVersion`, does not invalidate topology facts. It never compares Kubernetes resource-version strings numerically because they are opaque. This prevents a delayed result for a replaced UID from deleting a newer Node incarnation's facts. Within the same Node object, the provider's source generation rejects stale or contradictory source data.
...
At `OpenSession`, the scheduler captures a consistent cluster and topology view. Every topology Node fact records its Node UID; a Provider observation may also retain the Node `resourceVersion` as metadata. A bounded session-view coordinator captures the normal `SchedulerCache` snapshot and a topology snapshot without holding both cache locks at once, then compares the topology snapshot's Node identity map with the corresponding Nodes in the cluster snapshot. It retries at most two times on a mismatch. This comparison is indexed by Node name and does not scan individual devices.
...
An accepted `ReplaceFacts` update includes a source generation, timestamp, stable objects, and optional freshness deadline. An accepted `ClearFacts` update instead relies on its Node UID, while its `resourceVersion` remains observation metadata. The provider validates schema size, Node ownership, unique IDs, domain membership, fabric references, and source generation. The cache does not merge contradictory device facts from multiple authorities. Stale data makes devices unavailable for Required requests and removes the topology bonus for Preferred requests.
...
| Provider contract | `pkg/scheduler/topology/provider/provider.go` | Provider update contract, Node UID identity and observation metadata, per-Node UID initial-sync state, topology-content generation, liveness heartbeat and effective freshness deadline, structured validation result, and lifecycle. |
